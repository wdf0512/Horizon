...
def _resolve_test_url_to_public_ip():
    """Stub DNS resolution so safe_request()'s SSRF guard sees a public address.

    Without this, notify() resolves _TEST_URL's real hostname via live DNS,
    which is environment-dependent (e.g. sandboxes that intercept DNS).
    """
    with patch(
